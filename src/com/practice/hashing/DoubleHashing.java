...
public class DoubleHashing {
	...
	public void insertKeyInHashTable(String value) {
		...
		if (loadFactor >= 0.75) {
			...
			int firstHashValue = simpleASCIIHashFunction(value, hashTable.length);
			int secondHashValue = secondHashFunction(value, hashTable.length);
			for (int i = 0; i < hashTable.length; i++) {
				int newIndex = (firstHashValue + i * secondHashValue) % hashTable.length;
